...
@pytest.fixture
def quiet_console():
    """A rich Console that writes to /dev/null -- so pickers can run in CI
    without polluting test output."""
    from rich.console import Console

    return Console(file=open("/dev/null", "w"), quiet=True)
...
def test_pick_provider_lists_all(quiet_console, monkeypatch) -> None:
    """Every provider the picker offers shows up as a numbered option.

    We assert by introspecting the options list passed into ``_prompt_choice``,
    not the rendered table: the table is just a rendering of those options,
    so the option list IS the source of truth and is much easier to spy on.
    """
    from src.describe import all_providers
    from src import tui

    seen_options: list[list[tuple[str, str]]] = []
    real = tui._prompt_choice

    def spy(console, title, options, current, **kwargs):
        seen_options.append(options)
        # Return PICKER_KEEP so the picker exits cleanly.
        return PICKER_KEEP

    monkeypatch.setattr("src.tui._prompt_choice", spy)
    pick_provider(quiet_console, "anthropic")
    assert seen_options, "_prompt_choice was never called"
    providers_in_options = {value for value, _desc in seen_options[0]}
    for pid in all_providers():
        assert pid in providers_in_options, f"{pid} not in picker options"
...
def test_dispatch_provider_pick_writes_choice(quiet_console, tmp_path, monkeypatch) -> None:
    """Choosing provider option '3' (openai) writes provider=openai."""
    p = tmp_path / "config.yaml"
    p.write_text("provider: anthropic\n")
    monkeypatch.setattr("src.tui.Prompt.ask", lambda *a, **kw: "3")
    _dispatch("/provider", quiet_console, p)
    from src.config_writer import read_scalar
    assert read_scalar(p, "provider") == "openai"
